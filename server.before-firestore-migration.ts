...
// Simple JSON file based database for the QR codes
...
function getDB(): Database {
  ...
  if (!fs.existsSync(DB_FILE)) {
    db = { qr_codes: [], scans: [], users: [], sessions: [], admin_actions: [] };
  } else {
    db = JSON.parse(fs.readFileSync(DB_FILE, "utf-8"));
    if (!db.users) db.users = [];
    if (!db.sessions) db.sessions = [];
    if (!db.admin_actions) db.admin_actions = [];
  }

  let modified = false;
  if (!db.reset_tokens) { db.reset_tokens = []; modified = true; }

  // Migrate old statuses
  for (const qr of db.qr_codes) {
    if (qr.status === "PENDIENTE" as any) { qr.status = "UNACTIVATED"; modified = true; }
    if (qr.status === "ACTIVO" as any) { qr.status = "ACTIVE"; modified = true; }
    if (qr.status === "DESACTIVADO" as any) { qr.status = "DISABLED"; modified = true; }
  }

  // Seed demo QR codes
  const seedCodes = ["PR-11111", "PR-DEMO"];
  ...
  if (db.users.length === 0) {
    db.users = [
      { id: crypto.randomUUID(), username: "admin", name: "Admin", lastname: "Principal", email: "admin@puntoreview.com", phone: "", password: hashPassword("Picasso3@"), role: "ADMIN", account_status: "ACTIVE", subscription_status: "ACTIVE", created_at: new Date().toISOString() },
      { id: crypto.randomUUID(), username: "juanperez", name: "Juan", lastname: "Pérez", email: "juan@email.com", phone: "", password: hashPassword("Punto12345"), role: "RESELLER_ACTIVATOR", account_status: "ACTIVE", subscription_status: "NONE", created_at: new Date().toISOString() },
      { id: crypto.randomUUID(), username: "pedropro", name: "Pedro", lastname: "Gómez", email: "pedro@email.com", phone: "", password: hashPassword("Punto12345"), role: "RESELLER_PRO", account_status: "ACTIVE", subscription_status: "ACTIVE", created_at: new Date().toISOString() },
      { id: crypto.randomUUID(), username: "carloslopez", name: "Carlos", lastname: "López", email: "carlos@email.com", phone: "", password: hashPassword("Punto12345"), role: "RESELLER_PRO", account_status: "DISABLED", subscription_status: "ACTIVE", created_at: new Date().toISOString() },
    ];
    modified = true;
  }
...
function saveDB(db: Database) {
  ...
  saveQueue = saveQueue.then(() => fs.promises.writeFile(DB_FILE, data)).catch(console.error);
}
